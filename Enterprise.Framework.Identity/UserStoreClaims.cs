...
namespace Enterprise.Framework.Identity
{
    public partial class UserStore : IUserClaimStore<AppUser>, IEqualityComparer<Claim>
    {
        ...
        public Task<IList<AppUser>> GetUsersForClaimAsync(Claim claim, CancellationToken token) => Task.FromResult(Users.Where(u => u.Claims.Any(c => Equals(c, claim))).ToList() as IList<AppUser>);
